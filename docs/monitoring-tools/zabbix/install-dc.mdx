...
description: Install Zabbix with Docker Compose
---
...
Even for practicing, start at 2GB RAM. On my test setup (1GB RAM), MySQL container 1GB RAM ate up all the RAM out with the minimum monitoring when I try to open some graphs, then it caused the host to crash.
...
Choose a config from the following:

<Tabs>
  <TabItem value="basic" label="Basic config" default>

- This setup contains basic components to start zabbix: 
- **zabbix-db**, **zabbix-server**, **zabbix-web** and a **zabbix-agent** for local testing.

```yaml title="docker-compose.yml (basic)"
services:
  zabbix-db:
    image: mysql:8.0
...
```
  ...
  <TabItem value="with-web-service" label="With Web Service config">

- In addition to the **Basic config**, this setup adds **zabbix-web-service** and corresponding environment configuration..
- **zabbix-web-service** is for performing various tasks using headless web browser (for example, reporting).
  
```yaml title="docker-compose.yml (with web service)"
services:
  zabbix-db:
    image: mysql:8.0
    container_name: zabbix-db
  ...
  zabbix-server:
    ...
    environment:
      ...
      MYSQL_ROOT_PASSWORD: root_pass
      ZBX_SERVER_NAME: "Zabbix Docker"
      ZBX_STARTREPORTWRITERS: 1 # possible to enable from 1 to 100 processes.
  ...
  zabbix-web-service:
    image: zabbix/zabbix-web-service:alpine-7.2-latest
...
Note: the zabbix server IP may change after update
